...
import 'package:wbrs/pages/about_individual_meet.dart';
...
class _EditMeetState extends State<EditMeet> {
  ...
  void initState() {
    super.initState();
    name_meet.text = widget.meet.get('name');
    city.text = widget.meet.get('city');
    description.text = widget.meet.get('description');
    date_and_time.text = widget.meet.get('datetime');
  ...
  Widget build(BuildContext context) {
    ...
    FirebaseFirestore.instance.collection('meets').doc(widget.meet.id);
    return Stack(
      children: [
        Container(
                      ...
                      Row(
                        ...
                        children: [
                          Text(selectedTime.hour > 10
                              ...
                              : 'Время: 0${selectedTime.hour}:0${selectedTime.minute}',style: const TextStyle(color: Colors.white),),
                          ElevatedButton(
                              style: ElevatedButton.styleFrom(
                                  backgroundColor: Colors.orangeAccent),
                              onPressed: () {
                        ...
                        children: [
                          TextButton(
                              onPressed: () {
                                ...
                                if (name_meet.text.replaceAll(' ', '').isEmpty) {
                                ...
                                if (city.text == '') {
                                  ...
                                  collectionReference.update({
                                    ...
                                    'city': city.text.split(RegExp(r"(?! )\s{2,}")).join(' ').split(RegExp(r"\s+$")).join(''),
                          ...
                          TextButton(onPressed: (){
                            showDialog(context: context, builder: (BuildContext context) {
                              return AlertDialog(
                                ...
                                title: const Text('Удалить встречу?',style: TextStyle(color: Colors.white),),
                                titleTextStyle: const TextStyle(color: Colors.white),
                                actions: [
                                  TextButton(onPressed: (){Navigator.pop(context);}, child: const Text('Отмена',style: TextStyle(color: Colors.white),),),
                                  TextButton(onPressed: (){
                                    collectionReference.delete();
                                    nextScreenReplace(context, const MeetingPage());
                                  }, child: const Text('Удалить',style: TextStyle(color: Colors.redAccent, fontSize: 20)),),
                                ],
                              );
                            });
                          }, child: const Text('Удалить',style: TextStyle(color: Colors.redAccent, fontSize: 20)),),
