  ...
  Widget build(BuildContext context) {
    ...
    addNotification() async {
      Map notification = {
        ...
        'message': '${FirebaseAuth.instance.currentUser!.displayName} принял приглашение в группу',
      };
      String token = "";
      var meet = widget.snapshot.data.docs[widget.index];

      var doc = await FirebaseFirestore.instance.collection('TOKENS').doc(meet['admin']).get();
      token = doc.get('token');
      NotificationsService().sendPushMessageGroup(token, notification,meet['name'], 1, meet.id);
    }
    return Stack(
      children: [
        Image.asset(
        ...
        Scaffold(
          ...
          appBar: AppBar(
            ...
            actions: [isMeAdmin ? IconButton(onPressed: () {
              nextScreen(context, EditMeet(meet: widget.snapshot.data.docs[widget.index]  ));
            }, icon: const Icon(Icons.edit_calendar_outlined)) : const SizedBox()],
            backgroundColor: Colors.transparent,
            title: Text(widget.snapshot.data.docs[widget.index]['name'],style: TextStyle(color: Colors.white),),
          ),
          body: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const Center(
                  child: Text('Детали встречи',style: TextStyle(color: Colors.white),),
                ),
                const SizedBox(
                  height: 10,
                ),
                Text(
                    "Описание: ${widget.snapshot.data.docs[widget.index]['description']}",style: TextStyle(color: Colors.white),),
                const SizedBox(
                  height: 10,
                ),
                Text(
                    'Дата и время: ${widget.snapshot.data.docs[widget.index]['datetime']}',style: TextStyle(color: Colors.white),),
                const SizedBox(
                  height: 10,
                ),
                Text('Город: ${widget.snapshot.data.docs[widget.index]['city']}',style: TextStyle(color: Colors.white),),
                const SizedBox(
                  height: 50,
                ),
                isMeAdmin
                ...
                :widget.snapshot.data.docs[widget.index]['users']
                    ...
                    : Center(
                        child: ElevatedButton(
                            onPressed: () async {
                              DocumentReference meet = db
                                  .collection('meets')
                                  .doc(widget.snapshot.data.docs[widget.index].id);
                              List users =
                                  await meet.get().then((value) => value['users']);
                              meet.update({
                                'users': users + [MyUID]
                              });
                              String chatID = '';
                              bool haveChat = false;
                              String uid =
                                  await meet.get().then((value) => value['admin']);
                              ...
                              if (haveChat == false) {
                                ...
                                DatabaseService().addChat(
                                    FirebaseAuth.instance.currentUser!.uid, chatID);
                                DatabaseService().addChatSecondUser(uid, chatID);
